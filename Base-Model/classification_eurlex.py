...
class DataTrainingArguments:
    ...
    eval_split_ratio: Optional[float] = field(
        default=None,
        metadata={"help": "Ratio of training data to split into evaluation set."},
    )
...
def main():
    ...
    # Split the training set into train and evaluation based on the specified ratio
    # if data_args.eval_split_ratio is not None:
    #     logger.info(f"Splitting the training set with eval ratio: {data_args.eval_split_ratio}")
    #     split = raw_datasets['train'].train_test_split(test_size=data_args.eval_split_ratio, seed=42)
    ...
    if data_args.train_split_ratio is not None:
        ...
        split = raw_datasets['train'].train_test_split(test_size=data_args.train_split_ratio, seed=42)
        #train_dataset = train_validation_split['test']
        raw_datasets['train'] = split['test']
        if data_args.eval_split_ratio is not None:
            raw_datasets['validation'] = split['train'].train_test_split(test_size=data_args.eval_split_ratio, seed=42)['test']
    ...
    if training_args.do_train and not is_regression:  # classification, training
        ...
        logger.info("model config label2id: {}".format(model.config.label2id))
    ...
    def multi_labels_to_ids(labels: List[str]) -> List[float]:
        ...
        for label in labels:
            ...
            #ids[label_to_id[label]] = 1.0
            # sometimes label is int type and label_to_id= {'0': 0, '1': 1, ... 
            # in this case:
